...
class driveLoader(data.Dataset):
    ...
    def __init__(
        ...
        for split in ["train","test"]:
            path = pjoin(self.root, split)
            img_list = os.listdir(pjoin(path,"images"))
            if split=="train":
                mask_list = list(map(lambda x:x.replace('training','training_mask'),img_list))
                mask_list = list(map(lambda x:x.replace('.tif','.gif'),mask_list))
                label_list = list(map(lambda x:x.replace('training','manual1'),img_list))
                label_list = list(map(lambda x:x.replace('.tif','.gif'),label_list))
            else:
                mask_list = list(map(lambda x:x.replace('test','test_mask'),img_list))
                mask_list = list(map(lambda x:x.replace('.tif','.gif'),mask_list))
                label_list = list(map(lambda x:x.replace('test','manual1'),img_list))
                label_list = list(map(lambda x:x.replace('.tif','.gif'),label_list))
            self.imgfiles[split] = img_list
            self.maskfiles[split] = mask_list
            self.labelfiles[split] = label_list
            if not self.imgfiles[split]:
                raise Exception("No files for split=[%s] found in %s" % (split, self.root))
            print("Found %d %s images" % (len(self.imgfiles[split]), split))
    ...
    def __len__(self):
        """__len__"""
        return len(self.imgfiles[self.split])
    ...
    def __getitem__(self, index):
        ...
        im_name = self.imgfiles[self.split][index]
        lbl_name = self.labelfiles[self.split][index]
        mask_name = self.maskfiles[self.split][index]
        path = pjoin(self.root, self.split)

        img_path = pjoin(path, "images/" + im_name)
