...
class unetConv2_expand(nn.Module):
    def __init__(self, in_size, out_size, is_norm, is_groupnorm=True):
        super(unetConv2_expand, self).__init__()
        er = int(4)

        if is_norm:
            self.conv1 = nn.Sequential(
                # nn.Conv2d(in_size, out_size, 3, 1, 1),
                nn.Conv2d(in_size, in_size*er, 1, 1, 1),
                nn.Conv2d(in_size*er, out_size * er, 3, 1, 0),
                nn.Conv2d(out_size*er, out_size, 1, 1, 0),
                # nn.BatchNorm2d(out_size),
                nn.GroupNorm(min(8, out_size), out_size) if is_groupnorm else nn.BatchNorm2d(out_size),
                nn.ReLU(),
            )
            self.conv2 = nn.Sequential(
                # nn.Conv2d(out_size, out_size, 3, 1, 1),
                nn.Conv2d(out_size, out_size * er, 1, 1, 1),
                nn.Conv2d(out_size * er, out_size * er, 3, 1, 0),
                nn.Conv2d(out_size * er, out_size, 1, 1, 0),
                # nn.BatchNorm2d(out_size),
                nn.GroupNorm(min(8, out_size), out_size) if is_groupnorm else nn.BatchNorm2d(out_size),
                nn.ReLU(),
...
class unetUp_expand(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, is_batchnorm=False):
        super(unetUp_expand, self).__init__()
        self.conv = unetConv2_expand(in_size, out_size, is_batchnorm)
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=2, stride=2)
        else:
            self.up = nn.UpsamplingBilinear2d(scale_factor=2)
...
def weights_init_parametric_merge(w, input_channel, output_channel):
    w_size = w.data.size()
    assert output_channel == input_channel / 2
    a = torch.cat([torch.eye(output_channel), torch.eye(output_channel)])
    w.data = a.type(w.data.type()).view(*w_size)
    return w


class MergeParametric(nn.Conv2d):
    def __init__(self, bias=False, **kwargs):
        super(MergeParametric, self).__init__(bias=False, **kwargs)
        self.weight = weights_init_parametric_merge(self.weight, self.in_channels, self.out_channels)
